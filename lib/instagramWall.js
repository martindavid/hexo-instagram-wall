...
module.exports = function(namespace, config, logger) {
  ...
  if (fs.existsSync("cache/instagram.media.json")) {
    ...
    logger.info(
      "hexo-instagram-wall:",
      chalk.cyan("Instagram cache not found, cannot render instagram media.")
    );
    return render();
  }
  ...
  function render(media) {
    ...
    if (media && Array.isArray(media)) {
      ...
      wall.push(
        `<article class="js-instagram-wall instagram-wall" data-instagramWall="${namespace}">`
      );
      ...
      media.forEach(function(m) {
        ...
        let altText = m.caption ? m.caption.text.substring(0, 20) : "";

        wall.push(
          `<a href="${m.link}" target="_blank" rel="noopener noreferrer" class="instagram-wall__link">`
        );
        wall.push('<figure class="instagram-wall__brick low_resolution">');
        wall.push(
          `<img class="instagram-wall__media" alt=${altText} src="${imageObject.url}" width="${imageObject.width}" height=${imageObject.height}">`
        );

        if (currentConfig.showCaption && m.caption) {
